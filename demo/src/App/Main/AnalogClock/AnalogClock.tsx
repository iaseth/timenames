		...
		<section className="bg-white relative p-4">
			<section style={style} className="bg-zinc-100 w-full h-full rounded-full">
				<section className="absolute top-1/2 left-1/2 w-0 h-0">
					<ClockHand deg={h*30} color="#222" className="rounded-t-full" width={20} length={radius * 0.45} />
					<ClockHand deg={m*6} color="blue" className="rounded-t-full" width={15} length={radius * 0.70} />
					<ClockHand deg={s*6} color="red" className="rounded-t-full" width={10} length={radius * 0.85} />
					...
					{sixty.map(idx => <Dash key={idx} deg={idx * 6} radius={radius * 0.45} />)}
				</section>
			</section>
		</section>
	);
